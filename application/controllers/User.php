...
class User extends CI_Controller {
		...
		public function insert_user()
	{
		...
		$this->form_validation->set_rules('email', 'Email', 'required|xss_clean|is_unique[savsoft_users.email]');
        $this->form_validation->set_rules('password', 'Password', 'xss_clean|required');
	
		$this->form_validation->set_rules('first_name', 'Firstname', 'xss_clean|required');
		$this->form_validation->set_rules('last_name', 'Lastname', 'xss_clean');
		$this->form_validation->set_rules('contact_no', 'Contact Number', 'xss_clean');
		$this->form_validation->set_rules('subscription_expired', 'Subscription', 'xss_clean');
		...
		public function update_user($uid)
	{
		...
		$this->form_validation->set_rules('first_name', 'Firstname', 'xss_clean|required');
		$this->form_validation->set_rules('last_name', 'Lastname', 'xss_clean');
		$this->form_validation->set_rules('reg_no', 'Register Number', 'xss_clean');
		$this->form_validation->set_rules('contact_no', 'Contact Number', 'xss_clean');
           if ($this->form_validation->run() == FALSE)
                {
                     $this->session->set_flashdata('message', "<div class='alert alert-danger'>".validation_errors()." </div>");
	...
	public function insert_skills()
	{
		...
		$logged_in=$this->session->userdata('logged_in'); 
			...
			$this->load->library('form_validation');
		$this->form_validation->set_rules('tech_type', 'Type', 'xss_clean|required');
		$this->form_validation->set_rules('skills', 'Skills', 'xss_clean|required|is_unique[savsoft_skills.sub_skill_name]');
		...
		$this->load->view('header',$data);
                ...
                else
                {
					...
					if($this->user_model->insert_skills()){
                $this->session->set_flashdata('message', "<div class='alert alert-success'>".$this->lang->line('data_added_successfully')." </div>");
				}else{
					echo "in"; exit;
				 $this->session->set_flashdata('message', "<div class='alert alert-danger'>".$this->lang->line('error_to_add_data')." </div>");
	...
	public function skills(){
		...
		$data['skills']=$this->user_model->get_skills();
		$data['title']=$this->lang->line('group_list');
		$this->load->view('header',$data);
		$this->load->view('add_skills',$data);
		//$this->load->view('footer',$data);
		...
		// fetching group list
		$data['group_list']=$this->user_model->company_list();
		$data['title']=$this->lang->line('group_list');
		$this->load->view('header',$data);
...
//echo "in"; exit;
 ...
 redirect('login');
		...
		public function userlists(){
			...
			$skills['users']=$this->user_model->userslist();
	
	$this->load->view('header',$data);
			$this->load->view('userslist',$skills);
